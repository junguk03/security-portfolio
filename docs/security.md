...
> 2025년판 기준. 2021 대비 주요 변경: A02↔A05 위치 교체, A03→A05 재번호, A06:2021 Vulnerable Components → A03:2025 Software Supply Chain Failures, A10:2021 SSRF → A01:2025 Broken Access Control 통합, A10:2025 Mishandling of Exceptional Conditions 신설.
...
| A10 Mishandling of Exceptional Conditions | A12 에러 처리/로깅 (비정상 조건 처리 포함) |
...
- **OWASP**: A01 Broken Access Control (OWASP 2025에서 SSRF는 별도 카테고리 대신 Broken Access Control에 통합)
...
- [ ] Middleware에서 인증 우회 가능 경로 없는지 — Middleware만 믿지 말고 라우트 핸들러에서 재검증 필수 (CVE-2025-29927: `x-middleware-subrequest` 헤더 주입으로 Middleware 전체 우회 가능, CVSS 9.1, 14.2.25 / 15.2.3 미만 영향)

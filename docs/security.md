...
- **OWASP**: A01 Broken Access Control (2025에서 A10 SSRF가 A01로 통합)
...
- [ ] Next.js 12.3.5 / 13.5.9 / 14.2.25 / 15.2.3 이상 패치 여부 — `x-middleware-subrequest` 헤더를 통한 미들웨어 전체 우회 가능 (CVE-2025-29927, CVSS 9.1)
...
- [ ] 사용자 입력 딕셔너리를 `filter(**user_dict)` / `Q(**user_dict)` 에 직접 unpacking 금지 — ORM SQL Injection (CVE-2025-64459, CVSS 9.1)

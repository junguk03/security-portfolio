...
**nc (netcat) 명령어**
- `nc`: 네트워크 연결을 위한 유틸리티 (netcat)
- `localhost`: 현재 접속한 서버 자기 자신 (127.0.0.1)
- `30000`: 접속할 포트 번호

**포트(Port) 개념**
- 컴퓨터 안에서 포트는 **방(Room)** 같은 것
- 하나의 서버(건물)에 여러 개의 포트(방)가 있음
- 각 포트마다 다른 서비스가 실행됨
- 네트워크 통신은 "IP주소:포트번호"로 특정 서비스에 접속
...
**실무 연결**
- 서버 보안에서 불필요한 포트는 방화벽으로 차단
- 포트 스캐닝으로 열린 포트를 찾아 공격 벡터 탐색 가능
- `nmap` 같은 도구로 포트 스캔 수행
